...
const UpdatePlan = function(props) {
  ...
  const [annualprice, setAnnualPrice] = useState(props.annual_price || '');
  const [monthlyprice, setMonthlyPrice] = useState(props.monthly_price || '');
  const [maxfeeds, setMaxFeeds] = useState(props.max_feeds || '');
  const [maxusers, setMaxUsers] = useState(props.max_users || '');
  const [enablecustomfeeds, setEnableCustomFeeds] = useState(props.enable_custom_feeds || '');
  const [enableapi, setEnableApi] = useState(props.enable_api || '');
  const [enablecustomextraction, setEnableCustomExtraction] = useState(props.enable_custom_extraction || '');
  ...
  const savePlan = function() {
    let params = {
      ...
      max_feeds: maxfeeds,
      max_users: maxusers,
      enable_custom_feeds: enablecustomfeeds,
      enable_api: enableapi,
      enable_custom_extraction: enablecustomextraction,
    };
    let action;
    if (editMode) {
      params['id'] = id;
